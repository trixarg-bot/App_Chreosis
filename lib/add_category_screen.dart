  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Agregar Categoría', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
        centerTitle: true,
        elevation: 0,
              ...
              Form(
                ...
                child: Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Nombre de la categoría', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white)),
                      ...
                      const Text('Tipo', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white)),
const SizedBox(height: 8),
Row(
  children: [
    ...
    GestureDetector(
      onTap: () async {
        final iconoSeleccionado = await showDialog<IconData>(
          ...
          builder: (ctx) {
            return SimpleDialog(
              title: const Text('Selecciona un icono para la categoría'),
              children: listaIconosCategorias.map((icono) {
                return ListTile(
                  leading: Icon(icono['icon'], size: 32),
                  title: Text(icono['label']),
                  onTap: () => Navigator.pop(ctx, icono['icon']),
    ...
    const SizedBox(width: 14),
                          ChoiceChip(
                            ...
                            labelStyle: TextStyle(
                              color: _selectedType == 'Gasto' ? Colors.white : emerald,
                              fontWeight: FontWeight.bold,
                            ),
                            onSelected: (v) => setState(() => _selectedType = 'Gasto'),
                          ),
                          const SizedBox(width: 14),
                          ChoiceChip(
                            ...
                            backgroundColor: Colors.grey[800],
                            labelStyle: TextStyle(
                              color: _selectedType == 'Ingreso' ? Colors.black : emerald,
                              fontWeight: FontWeight.bold,
                            ),
                            onSelected: (v) => setState(() => _selectedType = 'Ingreso'),
                          ),
                        ],
                      ),
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          child: _loading
                              ...
                              : Text('Guardar', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.white)),
                        ),
                      ),
                    ],
                  ),
              ...
              Text('Tus categorías', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.white)),
              const SizedBox(height: 8),
              Expanded(
                child: Consumer<UserProvider>(
                  builder: (context, userProvider, _) {
                    ...
                    if (usuario == null) {
                      return const Center(child: Text('No hay usuario activo.'));
                    }
                    return FutureBuilder<List<Categoria>>(
                      future: DatabaseHelper.instance.getCategorias(userId: usuario.id!),
                      builder: (context, snapshot) {
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          return const Center(child: CircularProgressIndicator());
                        }
                        if (!snapshot.hasData || snapshot.data!.isEmpty) {
                          return const Center(child: Text('Aún no tienes categorías.', style: TextStyle(color: Colors.white)));
                        }
                        ...
                        return ListView.separated(
                          ...
                          itemBuilder: (context, i) {
                            ...
                            return ListTile(
                              ...
                              title: Text(cat.name, style: const TextStyle(fontWeight: FontWeight.w600, color: Colors.white)),
                              subtitle: Text(cat.type ?? '', style: const TextStyle(color: Color.fromARGB(255, 126, 125, 125))),
                              trailing: IconButton(
                                ...
                                onPressed: () async {
                                  ...
                                  if (confirm == true) {
                                    await DatabaseHelper.instance.deleteCategoria(cat.id!);
                                    setState(() {});
                                  }
                                },
                              ),
